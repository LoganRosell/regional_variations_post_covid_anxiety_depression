...
custom_palette = {
    'Anxiety': '#1f77b4',
    'Anxiety or Depression': '#d95f02',
    'Depression': '#2ca02c'
}

nat_avg_plt = sns.lineplot(
    national_avgs,
...
last_date = national_avgs['Time Period Start Date'].max()
offset = datetime.timedelta(days = 12)


for indicator in national_avgs['Indicator'].unique():
    subset = national_avgs[national_avgs['Indicator'] == indicator]
    last_val = subset[subset['Time Period Start Date'] == last_date]['nat_means'].values[0]
    
    ax.text(
        last_date + offset, 
        last_val, 
        indicator, 
        color=custom_palette[indicator],
        va='center', 
        fontsize=10, 
        fontweight='bold'
    )
events = [
    (floyd_protests, 'George Floyd Protests'),
    (federal_election, 'Federal Election'),
    (covid_vaccine, 'Covid-19 Vaccine Approved'),
    (roe_v_wade_overturned, 'Roe V. Wade is Overturned'),
    (us_mid_terms, 'Midterm Elections')
]

event_color = '#7f7f7f'  # Uniform neutral color for all event lines

# 2. Draw lines and add numbered circular tags
for idx, (date, label) in enumerate(events, start=1):
    ax.axvline(
        date,
        color=event_color,
        linestyle='--',
    ...
    # Place a numbered badge at the top of each vertical line (transform y to axes coords)
    ax.text(
        x=date,
        y=0.98,
        s=str(idx),
...
legend_labels = [f"[{i}]  {textwrap.fill(lbl, width=28)}" for i, (_, lbl) in enumerate(events, start=1)]

ax.legend(
    handles=[plt.Line2D([0], [0], color='none') for _ in events],  # invisible handles
...
plt.savefig("plots/fig_1_trends_over_time.png", bbox_inches='tight', dpi=300)
plt.show()
```
...
```{python}
#| echo: false
regional_diff = sns.lineplot(state_data[state_data["Indicator"] == "Anxiety or Depression"],
    x="Time Period Start Date",
...
plt.savefig("plots/fig_3_comparing_regions_over_time.png")
plt.show()
```
...
plt.savefig("plots/fig_4_linear_reg_by_region.png")
plt.show()
```
...
plt.savefig("plots/fig_5_resid_plot.png")
plt.show()
...
plt.savefig("plots/fig_6_QQ_plot.png")
plt.show()
