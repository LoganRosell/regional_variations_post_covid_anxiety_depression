...
events = [
    (floyd_protests, '#1b9e77', '--', 'George Floyd Protests'),
    (federal_election, '#d95f02', '-.', 'Federal Election'),
    (covid_vaccine, '#7570b3', '--', 'Covid-19 Vaccine Approved'),
    (roe_v_wade_overturned, '#e7298a', '-.', 'Roe V. Wade is Overturned'),
    (us_mid_terms, '#66a61e', '--', 'Midterm Elections')
]

for date, color, style, label in events:
    plt.axvline(
        date,
        color=color,
        label=label,
...
exclude = {'Anxiety', 'Depression', 'Anxiety or Depression'}
filtered_handles = [h for h, l in zip(handles, labels) if l not in exclude]
filtered_labels = [textwrap.fill(l, width=30) for l in labels if l not in exclude]
ax.legend(
