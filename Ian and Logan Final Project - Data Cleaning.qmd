...
```{python}
# Encoding dummy variables (keeps 'Region' for plots, adds 0/1 dummies for regression)
dummies = pd.get_dummies(state_data_wide['Region'], prefix='Region', drop_first=True, dtype=int)
state_data_wide = pd.concat([state_data_wide, dummies], axis=1)
```

```{python}
# shorten indicator names to make plotting cleaner later

# Ensure the column allows new string assignments
state_data["Indicator"] = state_data["Indicator"].astype(str)

# Update values in order (check 'or' first before replacing 'Anxiety')
state_data.loc[state_data["Indicator"].str.contains(r" or "), "Indicator"] = "Anxiety or Depression"
state_data.loc[state_data["Indicator"].str.contains(r"Depressive"), "Indicator"] = "Depression"
state_data.loc[state_data["Indicator"].str.contains(r"Anxiety Disorder"), "Indicator"] = "Anxiety"
```
